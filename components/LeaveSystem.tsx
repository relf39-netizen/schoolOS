...
import { 
    Clock, CheckCircle, XCircle, FilePlus, UserCheck, Printer, 
    ArrowLeft, Loader, Database, Calendar, User, ChevronRight, 
    Trash2, AlertCircle, Eye, Filter, X, Calculator, FileText,
    BarChart, Download, CalendarDays, Search, Timer, UserPlus, RefreshCw, ChevronDown, MapPin
} from 'lucide-react';
...
const LeaveSystem: React.FC<LeaveSystemProps> = ({ currentUser, allTeachers, currentSchool, focusRequestId, onClearFocus }) => {
    ...
    // Handle deep link from focusRequestId
    useEffect(() => {
                ...
                try {
                    ...
                    directors.forEach(dir => {
                        if (dir.telegramChatId) {
                            sendTelegramMessage(sysConfig.telegramBotToken!, dir.telegramChatId, message, link);
                        }
                    });
                } catch (tgError) {
