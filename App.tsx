...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const checkCalendarNotifications = async () => {
            ...
            try {
                // อัปเดต: ดึงการตั้งค่าจากพาธของโรงเรียน (Multi-school)
                ...
                snapshot.forEach(async (docSnap) => {
                    ...
                    if (evt.date === tomorrowStr && !evt.notifiedOneDayBefore) notifType = 'TOMORROW';
                    else if (evt.date === todayStr && !evt.notifiedOnDay) notifType = 'TODAY';
                    ...
                    if (notifType) {
                        ...
                        directors.forEach(d => {
                            if (config.telegramBotToken && d.telegramChatId) {
        ...
        if ('Notification' in window && Notification.permission === 'granted') {
            ...
            if (linkTo) {
                notif.onclick = (e) => {
                    ...
                    if (linkId) setFocusItem({ view: linkTo, id: linkId });
                ...
                if (currentUser.roles.includes('DIRECTOR')) {
                    ...
                    snapshot.docChanges().forEach((change) => {
                        if (change.type === 'added') {
                            ...
                            sendSystemNotification('อนุมัติการลา', msg, SystemView.LEAVE, change.doc.id);
                        }
                    });
                }
            });
    ...
    useEffect(() => {
        ...
        if (currentUser && isConfigured && db) {
            const qDocs = query(collection(db, "documents"), where("schoolId", "==", currentUser.schoolId));
            let isInitial = true;
            unsubDocs = onSnapshot(qDocs, (snapshot: QuerySnapshot<DocumentData>) => {
                ...
                snapshot.docChanges().forEach((change) => {
                    ...
                    if (change.type === 'added' && docData.status === 'PendingDirector' && currentUser.roles.includes('DIRECTOR')) {
                        ...
                        setNotification({ message: msg, type: 'info', linkTo: SystemView.DOCUMENTS, linkId: docId });
                    ...
                    if ((change.type === 'added' || change.type === 'modified') && docData.status === 'Distributed') {
                        ...
                        if (targets.includes(currentUser.id) && !acks.includes(currentUser.id)) {
                            ...
                            setNotification({ message: msg, type: 'alert', linkTo: SystemView.DOCUMENTS, linkId: docId });
    ...
    if (isSuperAdminMode) return <SuperAdminDashboard schools={allSchools} teachers={allTeachers} onCreateSchool={async(s)=>await setDoc(doc(db,'schools',s.id),s)} onUpdateSchool={handleUpdateSchool} onDeleteSchool={handleDeleteSchool} onUpdateTeacher={handleEditTeacher} onLogout={handleLogout} />;
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mt-8 flex justify-end">
                     <div className={`px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 ${isConfigured ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {isConfigured ? <Database size={14}/> : <ServerOff size={14}/>} {isConfigured ? 'System Online' : 'Offline Mode'}
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-slate-50 font-sarabun">
            {notification && (
                <div onClick={handleNotificationClick} className="fixed bottom-6 right-6 z-50 animate-slide-up print:hidden cursor-pointer">
            ...
            <header className="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-slate-200 shadow-sm print:hidden">
                <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-center md:justify-between relative">
                     <div className="absolute left-4 md:static flex items-center gap-2 md:gap-4">
                        {currentView !== SystemView.DASHBOARD && <button onClick={() => setCurrentView(SystemView.DASHBOARD)} className="p-2 hover:bg-slate-100 rounded-full text-slate-500"><ChevronLeft size={24} /></button>}
                    ...
                    <div className="absolute right-4 md:static flex items-center gap-4">
                        <div className="hidden md:flex flex-col items-end mr-2"><span className="text-sm font-bold text-slate-800">{currentUser.name}</span><span className="text-[10px] text-slate-500">{currentUser.position}</span></div>
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 py-3 px-6 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] print:hidden">
                <div className="max-w-7xl mx-auto flex justify-between items-center"><div className="flex items-center gap-2 text-slate-600"><Building2 size={18} className="text-blue-600"/><span className="font-bold text-sm md:text-base">{currentSchool?.name || 'SchoolOS System'}</span></div><div className="text-[10px] md:text-xs text-slate-400">© MR SIAM CH</div></div>
            </footer>
        </div>
    );
};
